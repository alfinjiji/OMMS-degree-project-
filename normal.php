...
<br/>
 ...
 <?php if(isset($_SESSION['user'])){ 
				...
				//$query = "SELECT * FROM orders WHERE customer_id='".$cusid."' ORDER BY id ";
				$query = $db->query("SELECT * FROM orders WHERE customer_id='".$cusid."' ORDER BY id");
				if($query->num_rows > 0){ 
				?>
				<?php
				$z=0;
					$sql1="SELECT * FROM orders WHERE customer_id='".$cusid."' ORDER BY id";
					$q1=$db->query($sql1);
...
<table class="table table-bordered">
			...
			<p><center>You have no orders.....</center></p>
			  <?php } }else { ?>
			  <p><center>Login first for order details.....</center></p>
			  <?php } ?>
			</tbody>
			<tfoot>
				<tr>	
